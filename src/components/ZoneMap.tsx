...
/** Point on the boundary ellipse at a bearing (radians, 0 = east). */
...
export default function ZoneMap({ zone }: { zone: Zone }) {
  const { path, exits, spots, terrain } = useMemo(() => {
    ...
    // Exits placed by real bearing toward each neighbor on the continent map.
    const exits = zone.connections
      ...
      .map((n, i) => {
        ...
        if (n.continent === zone.continent) {
          ...
          // Off-continent links (boats, portals) exit from the south rim.
    ...
    // Hotspots scattered deterministically inside the boundary.
    const spots = zone.hotspots.map((h, i) => {
      const angle = i * 2.39996 + rand() * 0.8; // golden angle spread
    ...
    <div className="zone-map-wrap">
      <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label={`Schematic map of ${zone.name}`}>
        ...
        <text x={CX} y={48} textAnchor="middle" fill="#a8988a" fontSize={11} fontFamily="Georgia, serif">
          {zone.type === 'city'
            ? 'City'
            : `Levels ${zone.levelMin}–${zone.levelMax}`}
        </text>
